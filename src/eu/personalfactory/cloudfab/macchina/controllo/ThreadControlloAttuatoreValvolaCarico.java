 ...
 */
public class ThreadControlloAttuatoreValvolaCarico extends Thread {

    private final Controllo controllo;

    //COSTRUTTORE
    public ThreadControlloAttuatoreValvolaCarico(Controllo controllo) {
    ...
    public void run() {
        ...
        ControlloLogger.logger.config("Inizio Controllo Attuatore Valvola di Carico");

        controllo.pannelloControllo.impostaMessaggioEsito(EstraiStringaHtml(TrovaVocabolo(ID_DIZIONARIO_MESSAGGI_MACCHINA, 473, ParametriSingolaMacchina.parametri.get(111))),
                EstraiStringaHtml(TrovaVocabolo(ID_DIZIONARIO_MESSAGGI_MACCHINA, 486, ParametriSingolaMacchina.parametri.get(111))));
        ...
        while (!controllo.avvio && controllo.pannelloControllo.isVisible()) {
            ...
            try {
                ThreadControlloAttuatoreValvolaCarico.sleep(
        ...
        if (controllo.testSuperato) {
            controllo.pannelloControllo.impostaMessaggioEsito(EstraiStringaHtml(TrovaVocabolo(ID_DIZIONARIO_MESSAGGI_MACCHINA, 473, ParametriSingolaMacchina.parametri.get(111))),
                    EstraiStringaHtml(TrovaVocabolo(ID_DIZIONARIO_MESSAGGI_MACCHINA, 473, ParametriSingolaMacchina.parametri.get(111)))
                    + " "
            ...
            //Memorizzazione Logger Procedure di Controllo
            ControlloLogger.logger.severe("Controllo Attuatore Valvola di Carico Fallito");
        ...
        //Memorizzazione Logger Procedure di Controllo
        ControlloLogger.logger.config("Fine Controllo Attuatore Valvola di Carico");
