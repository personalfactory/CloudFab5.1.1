 ...
 */
public class ThreadPuliziaApriValvolaCarico extends Thread {

    private Pulizia pulizia;

    public ThreadPuliziaApriValvolaCarico(Pulizia pulizia) {
    ...
    public void run() {
        ...
        while (timeToWait > 0 && pulizia.pannelloPulizia.isVisible()) {
            ...
            try {
                ThreadPuliziaApriValvolaCarico.sleep(1000);
        ...
        try {
            ThreadPuliziaApriValvolaCarico.sleep(Integer.parseInt(ParametriSingolaMacchina.parametri.get(429)));
        } catch (InterruptedException ex) {
            Logger.getLogger(ThreadPuliziaApriValvolaCarico.class.getName()).log(Level.SEVERE, null, ex);
        }
